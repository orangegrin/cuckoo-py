...
def get_maavg_value_route(request):
    ...
    ret_data = {}
    try:
        symbol_pair = request.raw_args["symbol_pair"] if request.raw_args.get(
            "symbol_pair", None) else request.json["symbol_pair"]
        redis_key = symbol_pair+":DIFFOFFSET"
        if request.method == "GET":
            ret_data = {"value": get_float_value_from_redis(
                redis_db, redis_key)}
...
def get_profit_range_value_route(request):
    ...
    try:
        symbol_pair = request.raw_args["symbol_pair"] if request.raw_args.get(
            "symbol_pair", None) else request.json["symbol_pair"]
        redis_key = symbol_pair+":PROFITRANGE"
        if request.method == "GET":
